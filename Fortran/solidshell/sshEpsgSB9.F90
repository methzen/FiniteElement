! --------------------------------------------------------------------
! Copyright (C) 1991 - 2020 - EDF R&D - www.code-aster.org
! This file is part of code_aster.
!
! code_aster is free software: you can redistribute it and/or modify
! it under the terms of the GNU General Public License as published by
! the Free Software Foundation, either version 3 of the License, or
! (at your option) any later version.
!
! code_aster is distributed in the hope that it will be useful,
! but WITHOUT ANY WARRANTY without even the implied warranty of
! MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
! GNU General Public License for more details.
!
! You should have received a copy of the GNU General Public License
! along with code_aster.  If not, see <http://www.gnu.org/licenses/>.
! --------------------------------------------------------------------
!
subroutine sshEpsgSB9(nb_node, nb_dof, geomInit, disp, zeta, epsg)
!
implicit none
!
#include "asterf_types.h"
#include "asterfort/assert.h"
#include "asterfort/sshTMatrDecoSB9.h"
#include "asterfort/sshGreenCovaSB9.h"
#include "asterfort/sshTMatrSB9.h"
!
integer, intent(in) :: nb_node, nb_dof
real(kind=8), intent(in) :: geomInit(3*nb_node), disp(nb_dof), zeta
real(kind=8), intent(out) :: epsg(6)
!
! --------------------------------------------------------------------------------------------------
!
! Solid-shell element - SB9
!
! Compute Green-Lagrange strains in cartesian frame at current point
!
! --------------------------------------------------------------------------------------------------
!
! In  nb_node          : number of nodes of element without pinch node(s)
! In  nb_dof           : number of dof
! In  geomInit         : initial coordinates of element
! In  disp             : displacements of element
! In  zeta             : out-of-plane parametric component
! Out epsg             : Green-Lagrange strains in cartesian frame at current point
!
! --------------------------------------------------------------------------------------------------
!
    real(kind=8) :: Ec0(6), EcZETA(6), EcZETAZETA(6)
    real(kind=8) :: TZETA(6,6), T0(6,6)
!
! --------------------------------------------------------------------------------------------------
!
    ASSERT(nb_node .eq. 8)
    ASSERT(nb_dof .eq. 25)
!
    epsg = 0.d0
!
! - Compute Green-Lagrange strains in covariant frame
!
    call sshGreenCovaSB9(nb_node , nb_dof,&
                         geomInit, disp  ,&
                         Ec0_ = Ec0, EcZETA_ = EcZETA, EcZETAZETA_ = EcZETAZETA)
!
! - Compute the decomposition of the inverse Jacobian matrix
!
    call sshTMatrDecoSB9(nb_node, geomInit, TZETA_ = TZETA)
!
! - Compute matrix relating the convective and Cartesian frame - Constant term
!
    call sshTMatrSB9(nb_node, geomInit, (/0.d0,0.d0,0.d0/), T0)
!
! - Compute Green-Lagrange strains in cartesian frame
!
    epsg = matmul(T0, Ec0)+&
          (matmul(T0, EcZETA)+matmul(TZETA, Ec0))*zeta+&
          (matmul(T0, EcZETAZETA)+matmul(TZETA, EcZETA))*zeta*zeta
!
end subroutine
